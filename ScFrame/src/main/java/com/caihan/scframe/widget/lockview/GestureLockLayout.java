...
public class GestureLockLayout extends RelativeLayout {
    ...
    //手指触摸是，path颜色
    private int mFingerTouchColor = Color.parseColor("#5987FF");
    //手指抬起时,密码匹配path颜色
    private int mFingerUpMatchedColor = Color.parseColor("#5987FF");
    //手指抬起时,密码不匹配path颜色
    ...
    private void setLockViewParams(ILockView lockView) {
        ...
        for (int i = 0; i < mDotCount * mDotCount; i++) {
            ...
            if (i % mDotCount != 0) {
                lockerParams.addRule(
                        RelativeLayout.RIGHT_OF,
                        mILockViews.get(i - 1).getView().getId());
            }
    ...
    private void handleResetMode() {
        if (mAnswerList.size() <= 0) {
            ...
            if (isAnswerRight) {
                ...
                toggleLockViewMatchedState(false);
                if (mOnLockResetListener != null) {
                    mOnLockResetListener.onSetPasswordFinished(
                            false, new ArrayList<Integer>(1));
                }
            }
        }
    }
    ...
    private boolean checkPointInChild(View childView, int x, int y) {
        ...
        if (x >= childView.getLeft() + padding &&
                x <= childView.getRight() - padding &&
