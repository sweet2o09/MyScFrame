...
public class NormalBottomHolder extends MyViewHolder<LocalShopCartButtomBean> {
    ...
    public void setData(LocalShopCartButtomBean item) {
        ...
        setText(R.id.total_goods_amount_tv, "商品总额：" + mRmbString + BaseParser.parseDoubleString(item.getItemTotalAmount()))
                .setGone(R.id.total_goods_amount_tv, item.getCartItemTradeType() != 0)
                .setText(R.id.tax_amount_tv, "税费：" + mRmbString + BaseParser.parseDoubleString(item.getTaxAmount()))
                .setGone(R.id.tax_amount_tv, item.getCartItemTradeType() != 0)
                ...
                .setChecked(R.id.settle_btn, item.isCanClick())
